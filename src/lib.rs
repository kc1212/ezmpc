...
mod tests {
    ...
    use crate::crypto::{Fp, combine, share};
    use crate::node::Node;
    use crate::message::*;
    ...
    fn create_sync_chans(
        ...
        let (from_sync, to_node) = vec![bounded(5); n].iter().cloned().unzip();
        let (from_node, to_sync) = vec![bounded(5); n].iter().cloned().unzip();
    ...
    fn integration_test_open() {
        ...
        let sync_handle = Synchronizer::spawn(sync_chans_for_sync.0, sync_chans_for_sync.1);
        let node_handles: Vec<JoinHandle<_>> = (0..n).map(move |i| {
            let node_handle = Node::spawn(
                i,
                sync_chans_for_node.0[i].clone(),
                sync_chans_for_node.1[i].clone(),
                triple_receiver.clone(),
                get_row(&node_chans, i).iter().map(|(s, _)| s.clone()).collect(),
                get_col(&node_chans, i).iter().map(|(_, r)| r.clone()).collect(),
                prog.clone(),
